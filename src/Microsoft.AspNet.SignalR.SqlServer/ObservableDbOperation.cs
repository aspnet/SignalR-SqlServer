...
namespace Microsoft.AspNet.SignalR.SqlServer
{
    ...
    internal class ObservableDbOperation : DbOperation, IDisposable, IDbBehavior
    {
        ...
        private DbOperation _notificationListen;
            ...
            : base(connectionString, commandText, logger, dbProviderFactory)
        {
            ...
            _logger = logger;
            _notificationListen = new DbOperation(connectionString, GetListenerSQL(), _logger);
        ...
        public void ExecuteReaderWithUpdates(Action<DbDataReader, DbOperation> processRecord)
        {
            ...
            var useNotifications = _dbBehavior.StartSqlNotificationListener();
...
#else
                                    _dbBehavior.AddSqlNotification(command, () => SqlNotificationListen(processRecord));
#endif
                                });
...
#else
        protected virtual void AddSqlNotification(DbCommand command, Action callback)
        {
            command.AddSqlNotification(() => callback());
        }

        [SuppressMessage("Microsoft.Design", "CA1031:DoNotCatchGeneralExceptionTypes", Justification = "On a background thread and we report exceptions asynchronously"),
         SuppressMessage("Microsoft.Usage", "CA1801:ReviewUnusedParameters", MessageId = "sender", Justification = "Event handler")]
        protected virtual void SqlNotificationListen(Action<DbDataReader, DbOperation> processRecord)
        {
            Logger.WriteInformation("{0}SQL notification change fired", LoggerPrefix);
            ...
            var result = _notificationListen.ExecuteScalar();
        ...
        protected virtual bool StartSqlNotificationListener()
        {
            lock (_stopLocker)
            {
                if (_disposing)
        ...
        bool IDbBehavior.StartSqlNotificationListener()
        {
            return StartSqlNotificationListener();
        }
...
#else
        void IDbBehavior.AddSqlNotification(DbCommand command, Action callback)
        {
            AddSqlNotification(command, callback);
        }
#endif
